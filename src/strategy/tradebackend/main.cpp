...
bool action(long int msgType, const void * data)
{
    ...
    if (msgType == MSG_TRADE_BUYOPEN) {
        service->tradeAction(TRADE_ACTION_BUYOPEN, msg.price, 1, msg.kIndex, string(msg.instrumnetID));
    }
    if (msgType == MSG_TRADE_SELLOPEN) {
        service->tradeAction(TRADE_ACTION_SELLOPEN, msg.price, 1, msg.kIndex, string(msg.instrumnetID));
    }
    if (msgType == MSG_TRADE_SELLCLOSE) {
        service->tradeAction(TRADE_ACTION_SELLCLOSE, msg.price, 1, msg.kIndex, string(msg.instrumnetID));
    }
    if (msgType == MSG_TRADE_BUYCLOSE) {
        service->tradeAction(TRADE_ACTION_BUYCLOSE, msg.price, 1, msg.kIndex, string(msg.instrumnetID));
    }
